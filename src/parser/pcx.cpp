...
namespace {
    auto readHeader(std::ifstream &file) -> std::optional<PCXHeader> {
        ...
        file.read(reinterpret_cast<char *>(&header.bpp), 1);
    ...
    inline auto hasRunMarker(uint8_t byte) -> bool {
    ...
    inline auto getRunCount(uint8_t byte) -> uint8_t {
    ...
    inline auto getImageDimensions(const PCXHeader &header) -> std::pair<int, int> {
...
auto PCXParser::loadPCX(const std::string &pakPath, const PakFileEntry &entry) -> std::optional<Texture> {
    ...
    // TODO: We should return a more generic chunk of data like rgba.data()
    // and remove the requirement for this to be coupled with OpenGL.
